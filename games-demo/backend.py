...
def generate_story(text_gen_prompt: str, response_schema) -> str:
    ...
    if response_schema is not None:
        config = GenerationConfig(
            temperature=temperature, max_output_tokens=max_output_tokens, response_schema=response_schema,response_mime_type="application/json",
        )
    else:
        config = GenerationConfig(
...
def generate_video(image, prompt):
  ...
  request_body = {
      "instances": [
          {
             "prompt": prompt,
            #   "image" : {"gcsUri" : "gs://bagchi-genai-bb/input_images/car-front.png" , "mimeType" :"png/jpg"}
          }
        ],
      "parameters": {
...
def initialize_backend():
    ...
    story = generate_story(
        "Tell me a fancy story with a beauty and a beast within 200 words set in modern times", None
    )
